...
namespace Lab1
{
	...
	bool MainForm::checkSelectedStatistics()
	{
		...
		{
			if (MessageBox::Show("Файл статистики не выбран, статистика записана не будет! Продолжить?", "Статистика",
	...
	System::Void MainForm::buttonEncrypt_Click(System::Object^  sender, System::EventArgs^  e)
	{
		if (checkSelectedStatistics())
		{
			...
			{
				MessageBox::Show("Ваш файл сообщения пуст! Шифрование невозможно.", "Ошибка чтения", MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
	}
	...
	System::Void MainForm::buttonDecrypt_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		{
			MessageBox::Show("Ваш файл шифротекста пуст! Дешифрование невозможно!", "Ошибка чтения", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
	...
	System::Void MainForm::buttonKasiski_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if (!checkEmptyFile(cipherTextFile))
		{
			...
			if (keyInfo->getLength() == 0)
				MessageBox::Show("Повторений не найдено", "Тест Касиски", MessageBoxButtons::OK, MessageBoxIcon::Information);
			else
			{
				form->Show();
		...
		{
			MessageBox::Show("Ваш файл шифротекста пуст! Проведение теста Касиски невозможно.", "Ошибка чтения", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
	...
	System::Void MainForm::buttonStats_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		while (!reader.eof())
		{
			...
			form->dataGridViewStats->Rows->Add(cipherTextLength.ToString(), realKeyLength.ToString(), possibleKeyLength.ToString(), stat == 1 ? "ТЕСТ ПРОЙДЕН" : "ТЕСТ НЕ ПРОЙДЕН");
		}
		...
		for (int i = 0; i < 4; i++)
		{
			if (lengthRange[i].amount != 0)
			{
				...
				case 2:
					...
					break;
				case 3:
					form->dataGridViewConclusion->Rows->Add("1000 и более", lengthRange[i].amount.ToString(), (lengthRange[i].success * 100 / lengthRange[i].amount).ToString() + "%");
					break;
				}
			}
		}
